...
export const GmailDraftModal: React.FC<GmailDraftModalProps> = ({
  ...
  const [visibleColumns, setVisibleColumns] = useState<DraftTableColumnKey[]>(() => {
    return DRAFT_AVAILABLE_COLUMNS.filter(c => c.defaultVisible).map(c => c.key);
  });
  ...
  }, [isOpen, selectedItems, activeViewTitle]);

  if (!isOpen) return null;

  const toggleColumn = (key: DraftTableColumnKey) => {
  ...
  const removeColumn = (key: DraftTableColumnKey) => {
    setVisibleColumns(prev => {
  ...
  const resetDefaultColumns = () => {
    setVisibleColumns(DRAFT_AVAILABLE_COLUMNS.filter(c => c.defaultVisible).map(c => c.key));
  };
              ...
              <div className="bg-slate-100/80 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 rounded-2xl p-3.5 space-y-2.5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-200">
                    ...
                    <span>Columnas de la Tabla ({visibleColumns.length}/{DRAFT_AVAILABLE_COLUMNS.length})</span>
                  </div>
                  <button
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {DRAFT_AVAILABLE_COLUMNS.map((col) => {
